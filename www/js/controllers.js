...
.controller('HomeCtrl', function ($scope, MyServices, $ionicModal, $location, $ionicPopup, $timeout, $stateParams, $ionicLoading, $interval, $cordovaImagePicker) {
    ...
    var isnewuserstatuschangecallback = function (data, status) {
        console.log(data);
    }

    $scope.closePassword = function () {
        console.log("in changeisnew");
        // IS NEW USER CHANGE STATUS
        MyServices.isnewuserstatuschange($.jStorage.get("user1"), isnewuserstatuschangecallback);
        $scope.oModal.hide();
    };
    ...
    $scope.memfunc = function (home) {
            ...
            if (purchasebalance > 0) {
                if (home.membershipno != "") {
                    ...
                    $location.url("/app/search/0/0/0/0");
                }
            } else {
    ...
    var balanceaddcallback2 = function (data, status) {
        console.log(data);
        $scope.orderidd = data;
        $scope.succurl = "http://wohlig.co.in/osb/index.php/json/payumoneysuccess?orderid=" + data
        if (data == "false") {
    ...
    $scope.openpayu = function (amount, reason) {
        $scope.user = $.jStorage.get("user1");
        $scope.a = amount;
        $scope.b = reason;
        $scope.allvalidation = [{
        ...
        if (check) {
            console.log("valid");
            console.log($scope.a, $scope.b);
            MyServices.balanceadd($scope.user, $scope.a, balanceaddcallback2, $scope.b);
    ...
    $scope.memfunc = function (home) {
        ...
        if (purchasebalance > 0) {
            if (home.membershipno != undefined && home.membershipno != "") {
                ...
                console.log(home);
                MyServices.searchresult(home.area, home.category, 0, 0, searchcallback);
            }
        } else {
            $scope.showPopupNoBalance();
        }
...
.controller('TransactionCtrl', function ($scope, $stateParams, $ionicPopup, $location, MyServices, $ionicLoading, $timeout, $ionicModal) {
    ...
    $scope.openpurchase = function (p) {
        if (p.reason.substr(0, 9) == 'ORDER ID:') {
            $scope.getp = p;
            $scope.modal.show();
        } else if (p.reason.substr(0, 9) != 'ORDER ID:') {
            $scope.getp = p;
            console.log($scope.getp);
            $scope.modal3.show();
        }
    };
    ...
    $scope.opensale = function (o) {
        if (o.reason.substr(0, 9) == 'ORDER ID:') {
            $scope.getsell = o;
            $scope.modal1.show();
        } else if (o.reason.substr(0, 9) != 'ORDER ID:') {
            $scope.getsell = o;
            console.log($scope.getsell);
            $scope.modal4.show();
        }
    };
    ...
    $ionicModal.fromTemplateUrl('templates/modaltransaction-purchasetransaction.html', {
        scope: $scope,
        animation: 'slide-in-up'
    }).then(function (modal) {
        $scope.modal3 = modal;
    ...
    $ionicModal.fromTemplateUrl('templates/modaltransaction-saletransaction.html', {
        scope: $scope,
        animation: 'slide-in-up'
    }).then(function (modal) {
        $scope.modal4 = modal;
...
.controller('ProfileCtrl', function ($scope, $stateParams, $ionicModal, $ionicPopup, $timeout, $ionicSlideBoxDelegate, MyServices, $http, $location, $ionicLoading, $cordovaCamera, $cordovaFileTransfer, $cordovaImagePicker) {
    ...
    $scope.changeshopimage = function (id) {
        ...
        $cordovaImagePicker.getPictures(options).then(function (resultImage) {
            ...
            $scope.cameraimage = resultImage[0];
            console.log(adminurl);
            $scope.uploadPhoto(adminurl + "imageuploadshop?id=" + id + "&user=" + $.jStorage.get("user1"), changeshopphoto);
    ...
    $scope.changeproductimage = function (id) {
        ...
        $cordovaImagePicker.getPictures(options).then(function (resultImage) {
            ...
            $scope.cameraimage = resultImage[0];
            console.log(adminurl);
            $scope.uploadPhoto(adminurl + "imageuploadproduct?id=" + id + "&user=" + $.jStorage.get("user1"), changeproductimagesuccess);
    ...
    $scope.uploadPhoto = function (serverpath, callback) {
        ...
        $cordovaFileTransfer.upload(serverpath, $scope.cameraimage, {})
    ...
    $scope.profileupdate = function () {
        ...
        MyServices.updateprofile($scope.id, $scope.profile, updateprofilecallback);
...
.controller('MyproductsCtrl', function ($scope, $stateParams, $ionicPopup, $ionicModal, $location, MyServices, $cordovaCamera, $timeout, $cordovaFileTransfer, $ionicLoading, $cordovaImagePicker) {
    ...
    $scope.addproductimage = function () {
        ...
        $cordovaImagePicker.getPictures(options).then(function (resultImage) {
            ...
            $scope.cameraimage = resultImage[0];
            console.log(adminurl);
            $scope.uploadPhoto(adminurl + "addproductimage?user=" + $.jStorage.get("user1"), addproductimage);
    ...
    $scope.editproductimage = function () {
        ...
        $cordovaImagePicker.getPictures(options).then(function (resultImage) {
            ...
            $scope.cameraimage = resultImage[0];
            console.log(adminurl);
            $scope.uploadPhoto(adminurl + "editproductimage?user=" + $.jStorage.get("user1"), editproductimage);
